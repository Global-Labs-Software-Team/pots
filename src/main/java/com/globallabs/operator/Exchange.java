...
public class Exchange implements ExchangeSpecification {
	...
	public void enrouteCall(final int phoneNumberFrom, final int phoneNumber) throws BusyPhoneException, PhoneNotFoundException {
		...
		if (phoneToCall == null || originPhone == null) {
			throw new PhoneNotFoundException("Either, the phone with id " + phoneNumber + "or" + phoneNumberFrom + " does not belongs to the network");
		}
	...
	public void openCallBetween(final int receiverNumber, final int callingNumber) throws NoCommunicationPathException {
		...
		if (!origin.equals(phone.getIncomingCall()) || !phone.equals(origin.getLastCall())) {
			throw new NoCommunicationPathException("There is no path between " + origin + " and " + phone);
		}
	...
	public void closeCallBetween(final int numberWhoCloseCall, final int theOtherNumberInCall) throws NoCommunicationPathException{
		...
		if (!(phoneWhoCloseCall.getLastCall() == phoneTheOtherEnd || phoneWhoCloseCall.getIncomingCall() == phoneTheOtherEnd)) {
			throw new NoCommunicationPathException("There is no path between " + phoneWhoCloseCall + " and " + phoneTheOtherEnd);
		}
